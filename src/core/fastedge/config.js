...
function prep() {
  ...
  if (!globalThis.fastedgeEnv) {
    globalThis.fastedgeEnv = new Map();
    if (process.env.CLOUD_PLATFORM)
      globalThis.fastedgeEnv.set('CLOUD_PLATFORM', process.env.CLOUD_PLATFORM)
    if (process.env.DISABLE_BLOCKLISTS)
      globalThis.fastedgeEnv.set('DISABLE_BLOCKLISTS', process.env.DISABLE_BLOCKLISTS)
    if (process.env.CF_DNS_RESOLVER_URL)
      globalThis.fastedgeEnv.set('CF_DNS_RESOLVER_URL', process.env.CF_DNS_RESOLVER_URL)
    if (process.env.LOG_LEVEL)
      globalThis.fastedgeEnv.set('LOG_LEVEL', process.env.LOG_LEVEL)
  }
